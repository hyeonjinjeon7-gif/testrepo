...
doc = """
설문 2-2. 사후설문 --- treatment(처치) 그룹 (그룹 1 --- 유연근무/일가정양립 컨설팅)
...
- 이 설문은 1 강연장(빨간 목줄)에만 비치하는 QR로 접속한다.
  F 블록만 그룹별로 다르고, 나머지 문항은 설문 2-1과 같다.
...
class C(BaseConstants):
    NAME_IN_URL = 'survey2_2_treatment'
    PLAYERS_PER_GROUP = None
    NUM_ROUNDS = 1

    # 이 설문이 담당하는 강연 그룹 (treatment: 그룹 1 --- 유연근무/일가정양립 컨설팅)
    GROUP = 1
...
class Player(BasePlayer):
    ...
    # ----------------------------------------------------------------- F. 남은 어려움 (그룹별)
    # 그룹 1 (유연근무) --- 설문 1 B10과 같은 보기
    f1g1_1 = check(DIFFICULTY_FLEX[0])
    f1g1_2 = check(DIFFICULTY_FLEX[1])
    f1g1_3 = check(DIFFICULTY_FLEX[2])
    f1g1_4 = check(DIFFICULTY_FLEX[3])
    f1g1_5 = check(DIFFICULTY_FLEX[4])
    f1g1_6 = check(DIFFICULTY_FLEX[5])
    f1g1_7 = check(DIFFICULTY_FLEX[6])
    f1g1_8 = check(DIFFICULTY_FLEX[7])
    f1g1_9 = check(DIFFICULTY_FLEX[8])
    f1g1_10 = check(DIFFICULTY_FLEX[9])
    f1g1_11 = check(DIFFICULTY_FLEX[10])
    f1g1_12 = check(DIFFICULTY_FLEX[11])
    f1g1_13 = check(DIFFICULTY_FLEX[12])
    f1g1_14 = check(DIFFICULTY_FLEX[13])
    f1g1_other = text('기타 (직접 입력)')

    f2g1_1 = check(C.F2_FLEX[0])
    f2g1_2 = check(C.F2_FLEX[1])
    f2g1_3 = check(C.F2_FLEX[2])
    f2g1_4 = check(C.F2_FLEX[3])
    f2g1_5 = check(C.F2_FLEX[4])
    f2g1_6 = check(C.F2_FLEX[5])
    f2g1_7 = check(C.F2_FLEX[6])
    f2g1_8 = check(C.F2_FLEX[7])
    f2g1_9 = check(C.F2_FLEX[8])
    f2g1_10 = check(C.F2_FLEX[9])
    f2g1_11 = check(C.F2_FLEX[10])
    f2g1_other = text('기타 (직접 입력)')
    ...
    h1 = radio(
        ...
        [[1, '예, 지금 아래에 적겠음'], [2, '예, 내일 연구팀 이메일에 회신하겠음'], [3, '아니오']],
    )
    h1_1 = radio(
...
F1_FIELDS = field_names('f1g1_', 14)
F2_FIELDS = field_names('f2g1_', 11)
...
class F(SurveyPage):
    ...
    form_model = 'player'
    form_fields = F1_FIELDS + ['f1g1_other'] + F2_FIELDS + ['f2g1_other']
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        if not getattr(player, F1_FIELDS[-1]):
            player.f1g1_other = None
        if not getattr(player, F2_FIELDS[-1]):
            player.f2g1_other = None
