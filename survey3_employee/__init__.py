...
class Player(BasePlayer):
    ...
    k5 = radio(  # 민감정보
        'K5. 현재 본인 또는 배우자가 임신 중입니까?',
        [[1, '예'], [2, '아니오'], [C.PREFER_NOT, '응답하지 않음']],
    )
    k6 = radio(
        'K6. 직무 유형',
        [
    ...
    k13 = number('K13. 월평균 가구소득 (만원)', min=1, max=9999)
    k13_1 = number('K13-1. 월평균 본인 노동소득 (만원)', min=0, max=9999)
    k13_2 = number('K13-2. 월평균 배우자 노동소득 (만원, 소득이 없으면 0)', min=0, max=9999)
    ...
    m2 = radio(  # 민감정보
        'M2. 지난 12개월 동안 직장에서 직장 내 괴롭힘(업무상 적정 범위를 넘어선 폭언이나 모욕, 따돌림, 부당한 업무 지시, 사적 심부름 등)을 경험하거나 목격한 적이 있습니까?',
        [
            [1, '본인이 직접 겪은 적이 있다'],
